...
impl KafkaSink {
	pub fn new(component_id: u32, server: impl Into<String>, port: u16, topic:  impl Into<String>, tx: Sender<ComponentMetrics>) -> Self {
		let server      = server.into();
		let topic       = topic.into();
		let created_utc = Instant::now();
		let started_utc = created_utc;
		let metrics     = ComponentMetrics::new(component_id);
		Self {component_id, server, port, topic, created_utc, started_utc, metrics, tx}
	}
...
impl ProvidesMetrics for KafkaSink {
